...
- **Send tab:** drop one file or a whole batch → get 12-char crypto-random code + `?code=...` link + locally-generated QR. Code expires after 30 min. Only one receiver at a time while live. Batches of 6+ files (any size), or 2+ files totaling 1+ GiB (and fitting the 2 GiB single-file / zip32 limits) auto-pack into one uncompressed `.zip`, so the receiver clicks a single download.
