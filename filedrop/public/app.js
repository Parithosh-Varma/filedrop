...
(function () {
  ...
  // --- Structured diagnostic logging (sender & receiver) ---
  // Track bytes sent vs. bytes received per file and per chunk.
  // Strip or gate behind a flag before production deploy if console noise is
  // unwanted.  Prefix every line with [FD-SEND] or [FD-RECV] so both tabs'
  // console output can be filtered side-by-side.
  var _fdDebug = false; // flip to true while diagnosing
  function _fdLog(side, msg) {
    if (!_fdDebug) return;
    try { console.log("[FD-" + side + "] " + msg); } catch (e) {}
  }
  ...
  // In-memory fallback (no OPFS: private mode, old browser) holds every chunk
  // plus a 2x Blob spike at seal — cap it well below MAX_FILE_SIZE so a 2 GiB
  // file cannot OOM the tab before the catch below runs.
  var MEMORY_MAX_BYTES = 512 * 1024 * 1024;
  ...
  function resetSendView() {
    ...
    // FX leftovers from a finished batch (100% stamp/glow) live on the ticket
    // node itself — strip them or the next batch inherits a bogus SENT badge.
    try {
      var ticket = sendPanel.querySelector(".ticket");
      if (ticket) {
        ticket.classList.remove("sent-glow");
        var stamps = ticket.querySelectorAll(".fx-stamp");
        for (var sti = 0; sti < stamps.length; sti++) {
          if (stamps[sti].parentNode) stamps[sti].parentNode.removeChild(stamps[sti]);
        }
      }
    } catch (e) {}
  ...
  function resetReceiveView() {
    ...
    // NOTE: vault-list URLs are deliberately NOT revoked here — the vault
    // persists across tab switches, and revoking without repainting would
    // leave its Download links dead. clearHistory() handles vault teardown.
    recvConns = [];
    rfiles = {}; rTotalBytes = 0; rDoneBytes = 0; rCount = 0;
    pendingBytes = 0; pendingCount = 0; rfileCount = 0; pendingWriteBytes = 0;
    try { vaultSaves.length = 0; } catch (e) {}
  ...
  function showSas(which, code, senderId, receiverId) {
    ...
    function paint(s, basic) {
      if (my !== sasToken) return; // superseded (new transfer / cleanup)
      var c = el.querySelector(".sas-code");
      // A non-crypto fallback is strictly weaker (32-bit FNV): label it so a
      // downgraded tab can never show the same string as a full-strength one.
      // Mixed pairs (SHA vs FNV) then visibly mismatch → users stop (fail-closed).
      if (c) c.textContent = basic ? s + " (basic)" : s;
      el.hidden = false;
    }
    try {
      if (window.crypto && crypto.subtle && crypto.subtle.digest && typeof TextEncoder !== "undefined") {
        crypto.subtle.digest("SHA-256", new TextEncoder().encode(input)).then(function (buf) {
          ...
          paint(("00" + Math.floor(n / 1000)).slice(-3) + "-" + ("00" + (n % 1000)).slice(-3), false);
        }).catch(function () { paint(fallbackSas6(input), true); });
        return;
      }
    } catch (e) {}
    paint(fallbackSas6(input), true);
  }
  function hideSas() {
    sasToken++;
    try {
  ...
  // descriptors) for maximum unzip compatibility. 1 MiB strides keep every
  // sync slice under ~10ms and report MB progress so multi-GB packs don't sit
  // on a static line; cancel is honored between strides.
  var PACK_STRIDE = 1048576;
  function packZipFiles(files, statusEl, done) {
    ...
    function nextFile() {
      ...
      (function stride() {
        if (stale()) return;
        var end = Math.min(off + PACK_STRIDE, f.size);
        f.slice(off, end).arrayBuffer().then(function (buf) {
          ...
          off = end;
          if (off < f.size) {
            statusEl.textContent = "Packing file " + (fi + 1) + " of " + files.length +
              " — " + fmt(off) + " / " + fmt(f.size) + "…";
            setTimeout(stride, 0);
          }
  ...
  function toast(msg, kind) {
    try {
      ...
      // per message per 3s so legit warnings are never evicted. Prune stale
      // keys so distinct filenames cannot grow the map for the tab lifetime.
      var now = Date.now();
      if (lastToastAt[msg] && now - lastToastAt[msg] < 3000) return;
      lastToastAt[msg] = now;
      var tkeys = Object.keys(lastToastAt);
      if (tkeys.length > 50) {
        for (var ti = 0; ti < tkeys.length; ti++) {
          if (now - lastToastAt[tkeys[ti]] > 10000) delete lastToastAt[tkeys[ti]];
        }
      }
  ...
  dz.addEventListener("keydown", function (e) {
    // The inner file input handles its own keys natively — only the bare
    // dropzone surface (now a non-focusable group) forwards Enter/Space.
    if (e.target && e.target.closest && e.target.closest("input,label")) return;
  ...
  function batchFingerprint(files) {
    try {
      // Full descriptor folded through FNV-1a (no raw truncation: long names
      // sharing a 2048-char prefix would otherwise collide). lastModified is
      // included so edited-same-name/same-size re-drops mint a fresh code.
      var raw = files.length + "|" + files.map(function (f) {
        return String(f.name || "file") + "|" + (f.size || 0) + "|" + (f.lastModified || 0);
      }).join(";");
      var h = 0x811c9dc5;
      ...
      return files.length + "|" + raw.length + "|" + h.toString(36);
    } catch (e) { return ""; }
  }
  ...
  function startSend(fileList) {
    ...
    for (var vi = 0; vi < files.length; vi++) {
      ...
      if (String(vf.name || "").length > MAX_NAME_LEN) {
        rejectBatch("File names must be shorter than " + MAX_NAME_LEN + " characters.");
        return;
      }
      total += vf.size;
  ...
  function startPeer(sendFiles, rows, code, peerId) {
    ...
    sendPeer.on("connection", function (conn) {
      ...
      _fdLog("SEND", "conn ACCEPTED from " + peer.slice(0, 20) + " total=" + (sendConns.length + 1));
      sendConns.push(conn);
      try {
  ...
  function onSendDead() {
    ...
    _fdLog("SEND", "onSendDead: all channels down, sendDoneBytes=" + sendDoneBytes + "/" + sendTotalBytes);
    sendAlive = false;
  ...
  function showResend() {
    try {
      var b = document.getElementById("send-again");
      b.hidden = false;
      // Keyboard focus was stranded on the now-irrelevant Stop button.
      if (!viewSend.hidden && document.activeElement &&
          document.activeElement.id === "send-cancel") b.focus();
    } catch (e) {}
  }
  ...
  function metaMsg(f, idx, count, payload, total) {
    ...
    _fdLog("SEND", "meta fi=" + idx + " name=" + m.name.slice(0, 40) + " size=" + f.size + " chunk=" + payload + " total=" + total + " tb=" + sendTotalBytes);
    return m;
  }
  ...
  async function pump(files, rows) {
    ...
    lastPayload = payload;
    _fdLog("SEND", "pump START files=" + files.length + " totalBytes=" + totalBytes + " payload=" + payload + " openConns=" + openSendConns().length);
    ...
    async function ensureOpen() {
      for (var t = 0; t < 40; t++) {
        if (genStale() || !sendAlive) { _fdLog("SEND", "ensureOpen: stale/dead, attempt=" + t); return false; }
        if (openSendConns().length) return true;
        await new Promise(function (r) { setTimeout(r, 250); });
      }
      _fdLog("SEND", "ensureOpen: TIMEOUT after 10s, openConns=" + openSendConns().length);
    ...
    while (true) {
      ...
      try {
        var _chunkBuf = frameChunk(item.fi, item.i, item.buf);
        _fdLog("SEND", "chunk fi=" + item.fi + " i=" + item.i + " payload=" + item.buf.byteLength + " framed=" + _chunkBuf.byteLength + " sentOk=true inflight=" + inflightBytes());
        cons[0].send(_chunkBuf);
        sendFailCount = 0;
      } catch (e) {
        _fdLog("SEND", "chunk fi=" + item.fi + " i=" + item.i + " FAILED: " + (e && e.message || e));
        queue.unshift(item);
  ...
  var pendingWriteBytes = 0; // bytes queued in OPFS writeChain, not yet on disk
  var WRITE_QUEUE_MAX = 512 * 1024 * 1024; // abort past this: crash is worse
  ...
  function checkRecvRateLimit() {
    var now = Date.now();
    // Reload-surviving budget: attempts persist in sessionStorage so a reload
    // loop cannot reset the 5/min window (new tabs still get a fresh budget —
    // static-only has no cross-tab store; the 12-char space remains the real
    // defense).
    try {
      var saved = JSON.parse(store.get("fd-rl") || "[]");
      if (saved instanceof Array) {
        for (var si = 0; si < saved.length; si++) {
          var st = saved[si];
          if (typeof st === "number" && now - st < RECV_WINDOW_MS && recvAttempts.indexOf(st) === -1) {
            recvAttempts.push(st);
          }
        }
      }
    } catch (e) {}
    ...
    if (recvAttempts.length >= RECV_MAX_ATTEMPTS) {
      try { store.set("fd-rl", JSON.stringify(recvAttempts)); } catch (e) {}
  ...
  function recvCtrlSend(msg) {
    // Least-buffered first: progress/acks must not queue behind megabytes on
    // one slow channel while the others sit idle (sender mirrors stall).
    var best = null, bestBuf = Infinity;
    for (var i = 0; i < recvConns.length; i++) {
      var c = recvConns[i];
      if (!connOpen(c)) continue;
      var b = connBuffered(c);
      if (b < bestBuf) { bestBuf = b; best = c; }
    }
    if (!best) return false;
    try { best.send(msg); return true; } catch (e) {}
    return false;
  }
  ...
  function connectRecv(code) {
    ...
    // OPFS temp namespace is origin-shared across tabs: crypto-random suffix
    // (not 1/1296 Math.random) so two tabs starting in the same ms cannot
    // interleave positional writes into each other's temp files.
    try {
      var rs = new Uint32Array(2);
      window.crypto.getRandomValues(rs);
      recvSession = Date.now().toString(36) + rs[0].toString(36) + rs[1].toString(36);
    } catch (e) {
      recvSession = Date.now().toString(36) + Math.floor(Math.random() * 1296).toString(36) + Math.floor(Math.random() * 1296).toString(36);
    }
    recvOpfsRoot = null; recvOpfsTried = false;
    rGrandTotal = 0; lastProgSent = 0;
    allDoneMsg = false; firstMetaSeen = false; lastRecvUi = 0;
    ...
    // No-answer watchdog: signaling is up (SAS shows) but the sender's data
    // channels may never open (closed tab, stale code, ICE dead-end). Turn a
    // silent "Connecting…" hang into an actionable message instead.
    var myRecvSession = recvSession;
    var noAnswerTimer = setTimeout(function () {
      if (myRecvSession !== recvSession || recvAborted) return;
      if (recvConns.some(connOpen)) return;
      setRecv("Still no answer from the sender (25s). Is their Send tab still open with this exact code? Codes expire after 30 min — ask them to re-drop for a fresh code, then tap Reconnect.");
      ...
      for (var k = 0; k < NUM_CHANNELS; k++) {
        (function (k) {
          ...
          conn.on("open", function () {
            opened++;
            _fdLog("RECV", "conn[" + k + "] OPEN (opened=" + opened + "/" + NUM_CHANNELS + ")");
            if (opened === 1) setRecv("Connected — waiting for the first file…", 0);
            if (noAnswerTimer) { try { clearTimeout(noAnswerTimer); } catch (e) {} noAnswerTimer = null; }
          });
          conn.on("data", onData);
          conn.on("close", function () {
            _fdLog("RECV", "conn[" + k + "] CLOSE");
            onRecvDead();
          });
          conn.on("error", function (err) {
            _fdLog("RECV", "conn[" + k + "] ERROR: " + (err && (err.type || err.message) || err));
          });
        })(k);
      }
    });
    recvPeer.on("error", function (err) {
      if (noAnswerTimer) { try { clearTimeout(noAnswerTimer); } catch (e2) {} noAnswerTimer = null; }
  ...
  function setupOpfs(f, meta) {
    f.opfsReady = getOpfsRoot().then(function (root) {
      ...
      if (!ok) {
        ...
        if (meta.size > MEMORY_MAX_BYTES) { abortRecv("Transfer stopped: this browser cannot stream to disk and the file is too large to hold in memory — try a smaller file or a browser with OPFS support."); return ok; }
        if (!f.chunks) {
  ...
  function storeChunk(f, i, payload) {
    ...
    if (payload.byteLength > MAX_PAYLOAD + MAX_BIN_SLOP) {
      _fdLog("RECV", "storeChunk REJECT fi=" + f.fi + " i=" + i + " size=" + payload.byteLength + " max=" + (MAX_PAYLOAD + MAX_BIN_SLOP));
    ...
    if (exp < 0 || payload.byteLength !== exp) {
      _fdLog("RECV", "storeChunk SIZE_MISMATCH fi=" + f.fi + " i=" + i + " got=" + payload.byteLength + " expected=" + exp + " f.chunk=" + f.chunk + " f.total=" + f.total + " f.size=" + (f.meta && f.meta.size));
      abortRecv("Transfer stopped: invalid chunk size from sender.");
      return;
    }
    if (f.useOpfs || (f.opfsReady && !f.chunks)) {
      ...
      f.written[i] = true;
      pendingWriteBytes += payload.byteLength;
      if (pendingWriteBytes > WRITE_QUEUE_MAX) {
        abortRecv("Transfer stopped: local storage cannot keep up with the sender — try again on this device.");
        return;
      }
      ...
      f.writeChain = f.writeChain.then(function () {
        ...
        pendingWriteBytes = Math.max(0, pendingWriteBytes - payload.byteLength);
        if (f.gen !== recvGen || recvAborted || f.complete) return;
        f.got++;
        rDoneBytes += payload.byteLength;
        _fdLog("RECV", "storeChunk OPFS_OK fi=" + f.fi + " i=" + i + " got=" + f.got + "/" + f.total + " rDoneBytes=" + rDoneBytes + "/" + rGrandTotal);
        // Skip the fmt() string build when the 150ms UI throttle will drop
        // this tick anyway (per-chunk hot path).
        if (Date.now() - lastRecvUi >= UI_MS) {
    ...
    if (!f.chunks[i]) {
      ...
      f.got++;
      rDoneBytes += payload.byteLength;
      _fdLog("RECV", "storeChunk MEM_OK fi=" + f.fi + " i=" + i + " got=" + f.got + "/" + f.total + " rDoneBytes=" + rDoneBytes + "/" + rGrandTotal);
      if (Date.now() - lastRecvUi >= UI_MS) {
  ...
  function onData(d) {
    ...
    _fdLog("RECV", "onData type=" + typeof d + " constructor=" + (d && d.constructor && d.constructor.name) + " byteLen=" + (d && d.byteLength != null ? d.byteLength : d && d.size != null ? d.size : "?"));
    // PeerJS may deliver data in several shapes depending on browser/version:
    // ArrayBuffer, Uint8Array, Blob, or a wrapped object { unwrap:true, data:... }.
    // Handle every variant so binary chunks are never silently dropped.
    ...
    // PeerJS 1.5.x sometimes wraps messages: { unwrap: true, data: <payload> }.
    if (d && typeof d === "object" && d.unwrap && d.data != null) {
      _fdLog("RECV", "unwrapping PeerJS envelope");
      onData(d.data);
      return;
    }
  ...
  function onBinary(buf) {
    if (recvAborted) return;
    if (!buf || buf.byteLength < HEADER) { _fdLog("RECV", "onBinary: short buf byteLen=" + (buf ? buf.byteLength : 0)); return; }
    ...
    _fdLog("RECV", "onBinary fi=" + fidx + " i=" + i + " payload=" + payload.byteLength + " totalBuf=" + buf.byteLength);
  ...
  function validMeta(d) {
    ...
    if (!Number.isFinite(d.chunk) || Math.floor(d.chunk) !== d.chunk || d.chunk < MIN_CHUNK || d.chunk > MAX_PAYLOAD + MAX_BIN_SLOP) return "bad chunk size";
  ...
  function onControl(d) {
    if (recvAborted) return;
    if (d.t === "meta") {
      _fdLog("RECV", "control META fi=" + d.fi + " name=" + String(d.name || "").slice(0, 40) + " size=" + d.size + " chunk=" + d.chunk + " total=" + d.total + " tb=" + d.tb);
      var err = validMeta(d);
      ...
      _fdLog("RECV", "control FDONE fi=" + d.fi);
      ...
      _fdLog("RECV", "control CANCELLED");
      onSenderCancelled();
    } else if (d.t === "done") {
      _fdLog("RECV", "control DONE rDoneBytes=" + rDoneBytes + "/" + rGrandTotal);
      allDoneMsg = true;
      maybeFinishAll();
    }
  }
  ...
  function sealOpfsFile(f) {
    ...
    f.writer.truncate(f.meta.size).then(function () {
      ...
      f.complete = true;
      // Drop OPFS temp handle ASAP; blob holds the bytes. Re-wrap risky
      // payloads as octet-stream (same rule as the memory path) so the MIME
      // invariant holds no matter which path sealed the file.
      var tmpName = f.opfsName;
      f.writer = null;
      deleteTempOpfs(tmpName);
      var safe = file;
      try {
        if (riskyKind(f.meta.name)) safe = new Blob([file], { type: "application/octet-stream" });
      } catch (e) { safe = file; }
      finishWithBlob(f, safe);
      maybeFinishAll();
    }).catch(function () {
      if (f.gen !== recvGen) return;
      if (!recvAborted) {
  ...
  function maybeFinishAll() {
    ...
    if (doneStallTimer) { try { clearTimeout(doneStallTimer); } catch (e) {} doneStallTimer = null; }
    var finishGen = recvGen, finished = false;
    function finishDone() {
      if (finished || recvAborted || finishGen !== recvGen) return;
      finished = true;
      document.getElementById("receive-cancel").hidden = true;
      document.getElementById("receive-reconnect").hidden = true;
    ...
    // Vault commits are async: hold Done (and the beforeunload guard via
    // transferActive) until they settle, so "saved below" is already true.
    // Bounded 5s fallback — a wedged IDB must never strand the transfer.
    var pending = vaultSaves.slice();
    if (!pending.length) { finishDone(); return; }
    setRecv("Saving files on this device…", 99);
    var vto = setTimeout(finishDone, 5000);
    Promise.all(pending).then(function () { clearTimeout(vto); finishDone(); },
      function () { clearTimeout(vto); finishDone(); });
  }
  ...
  var vaultEpoch = 0;
  // Pending vault commits for the current session. maybeFinishAll waits for
  // these (bounded) before printing Done, so closing the tab on "Done" cannot
  // lose the IDB commit it promises ("copies are also saved below").
  var vaultSaves = [];
  function vaultOpen() {
    return new Promise(function (resolve) {
  ...
  function saveVaultFile(meta, blob) {
    try {
      ...
      if (document.getElementById("vault-off") && document.getElementById("vault-off").checked) return Promise.resolve(false);
    } catch (e) {}
    var safeName = sanitizeDownloadName(meta.name);
    if (blob.size > VAULT_MAX_BYTES) return Promise.resolve(false); // never vault huge files
    var myEpoch = vaultEpoch;
    var p = vaultOpen().then(function (db) {
      if (!db || myEpoch !== vaultEpoch) return false;
      return new Promise(function (resolve) {
        try {
        var tx = db.transaction("files", "readwrite");
        ...
        allReq.onsuccess = function () {
          try {
            if (myEpoch !== vaultEpoch) { resolve(false); return; }
            ...
            if (total + blob.size > VAULT_MAX_BYTES) { resolve(false); return; }
            os.add({
              name: safeName,
    ...
    vaultSaves.push(p);
    if (vaultSaves.length > 200) vaultSaves.splice(0, vaultSaves.length - 200);
    return p;
  }
  ...
  // In-place resend bridge for ui.js: a fresh send view without a full page
  // reload (vault/IDB untouched, observers stay wired). Fallback is reload.
  try {
    window.FDReset = { send: function () {
      resetSendView();
      show("send");
