...
(function () {
  ...
  var MAX_CONFETTI = 400; // overlapping celebrations cannot grow the layer forever
  function burst(x, y, n) {
    var layer = $("fx-confetti");
    if (!layer || motionReduced()) return;
    n = n || 70;
    if (layer.childNodes.length > MAX_CONFETTI) return;
  ...
  (function splitWordmark() {
    ...
    // role="img" so AT name computation honors the label while the split
    // chars below stay hidden.
  ...
  (function entrance() {
    ...
    try {
      ...
      var tl = hasGsap.timeline({ defaults: { ease: "power3.out", clearProps: "opacity,transform" } });
  ...
  (function ambient() {
    ...
    if (A) {
      try {
        ambientAnims.push(A({ targets: ".fx-plane.p1", translateX: [0, 60, 0], translateY: [0, -26, 0], rotate: [0, 12, 0], duration: 11000, loop: true, easing: "easeInOutSine" }));
        ambientAnims.push(A({ targets: ".fx-plane.p2", translateX: [0, -48, 0], translateY: [0, 22, 0], rotate: [0, -10, 0], duration: 14000, loop: true, easing: "easeInOutSine", delay: 800 }));
        ambientAnims.push(A({ targets: ".fx-plane.p3", translateX: [0, 36, 0], translateY: [0, -18, 0], rotate: [0, 8, 0], duration: 17000, loop: true, easing: "easeInOutSine", delay: 1600 }));
        ambientAnims.push(A({ targets: ".counter-icon", translateY: [0, -5, 0], duration: 2600, loop: true, direction: "alternate", easing: "easeInOutSine" }));
      } catch (e) {}
    }
  ...
  // Mid-session toggle: the load-time check above cannot see an OS setting
  // flipped while the page is open. Pause the infinite ambient loops, hide
  // the ambient layer, drain confetti; discrete CSS already adapts itself.
  // Ticket/status tweens are left alone (pausing one mid-flight could stick
  // opacity mid-fade; they are short-lived anyway).
  try {
    ...
    if (rmq && rmq.addEventListener) rmq.addEventListener("change", function (e) {
      if (!e.matches) return;
      try {
        for (var i = 0; i < ambientAnims.length; i++) {
          try { if (ambientAnims[i] && ambientAnims[i].pause) ambientAnims[i].pause(); } catch (_) {}
        }
      } catch (_) {}
      try { if (hasGsap) hasGsap.killTweensOf(".fx-ambient"); } catch (_) {}
      try { var amb = document.querySelector(".fx-ambient"); if (amb) amb.style.display = "none"; } catch (_) {}
      try { var cf = $("fx-confetti"); if (cf) cf.innerHTML = ""; } catch (_) {}
    });
  } catch (e) {}
  ...
  function scrambleCode(el) {
    ...
    // Unwrap a previous share's .code-char spans so a different code for a
    // new batch re-scrambles from plain text instead of writing over spans
    // that popChars would then refuse to re-wrap.
    if (el.querySelector(".code-char")) {
      el.textContent = Array.prototype.map.call(
        el.querySelectorAll(".code-char"), function (s) { return s.textContent; }
      ).join("");
    }
  ...
  function animateNewRows(ul) {
    ...
    // Animate only the rows inserted since the last pass — previously the
    // trailing six replayed on every insertion.
    if (A && fresh.length) {
      try {
  ...
  function watchProgress(pctId, barId, liveId) {
    ...
    new MutationObserver(function () {
      ...
      if (bar && liveId) {
        ...
        // Dirty-check: per-tick closest()+toggle was pure waste when `on`
        // hadn't changed since the previous tick.
        if (on !== lastFlow) {
          lastFlow = on;
  ...
  (function sendDone() {
    ...
    new MutationObserver(function () {
      ...
      if (!Number.isFinite(v) || v < 100 || fired) return;
      fired = true;
