...
- `app.js` + inline script in `index.html` share DOM state (send `100%` → show resend; receive `<progress hidden>` → wrapper). Keep IDs stable: `tab-send/receive`, `view-send/receive`, `dropzone`, `file-input`, `send-panel`, `share-code/link/qr`, `send/receive-progress`, `send/receive-pct`, `send/receive-status`, `send/receive-filelist`.
